<template>
  <c-dropdown>
    <c-dropdown-button
      class="w-full dark:hover:bg-gray-800 transition-all rounded-lg"
    >
      <div class="px-2">
        <!--        <small class="block dark:text-gray-400 text-left">{{ props.type }}</small>-->
        <div class="flex items-center gap-4">
          <span class="text-gray-200 text-sm flex-shrink-0">{{
            props.selected?.name || props.noSelectedMessage
          }}</span>
          <ChevronUpDownIcon
    ...
    <c-dropdown-list style="left: 0; min-width: 200px">
      <div class="flex items-center gap-2 w-full">
        ...
        <input
          class="outline-none focus:ring-0 border-none w-full text-sm p-0"
      ...
      <div>
        <c-dropdown-item
          class="flex-shrink-0 text-sm text-primary"
          @click="$emit('add')"
          >+ Ajouter un projet</c-dropdown-item
        >
      </div>
    </c-dropdown-list>
  </c-dropdown>
