<template>
  <Listbox v-model="selectedItem" v-if="selectedItem">
    <div class="relative">
      <ListboxButton
        class="relative h-full text-start px-5 py-5 flex gap-4 w-full justify-between rounded-md border-0 shadow-sm ring-1 ring-inset sm:text-sm/6 ring-gray-300 placeholder:text-gray-400 dark:ring-gray-800 h-full"
      >
        <div>
          <span class="text-sm block dark:text-gray-500"
            >Nom du compte de facturation</span
          >
          <span class="text-lg font-semibold">{{ selectedItem.name }}</span>
        </div>
        <div>
          <Icon
            :name="`solar:round-double-alt-arrow-down-bold-duotone`"
            size="32px"
            class="text-gray-400"
          />
        </div>
        ...
        >
          ...
          >
            <li
              :class="[
                active
                  ? 'bg-blue-100 text-blue-900 dark:bg-primary dark:text-white'
                  : 'text-gray-900',
          ...
          <div class="mb-2 mx-2">
            ...
            <div
              ...
              :class="[
                'hover:bg-gray-100 hover:dark:bg-gray-800 hover:outline-none',
                'block px-3 py-1 text-sm/6 text-gray-900 rounded-lg dark:text-gray-400 cursor-pointer',
              ]"
            >
