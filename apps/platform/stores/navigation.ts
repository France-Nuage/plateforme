...
export const useNavigationStore = defineStore("navigation", {
  ...
  actions: {
    ...
    loadOrganizations: async function (queryParams: any = null) {
      ...
      return $api()
        ...
        .then((response) => {
          ...
          this.organization = response.data?.length ? response.data[0] : null;
    ...
    selectOrganization: async function (id: string) {
      ...
      return $api()
        .organizations.get(id, { includes: "folders.projects" })
        .then((response) => {
    ...
    selectFolder: async function (id: string) {
      ...
      return $api()
        .folders.get(id, { includes: "projects" })
        .then((response) => {
          this.folder = response;
          this.projects = this.folder.projects
            ? this.folder.projects.map((item) => ({
                ...item,
