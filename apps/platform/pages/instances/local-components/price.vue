<template>
  <div class="bg-white shadow sm:rounded-lg dark:bg-gray-900">
    <div class="px-4 py-5 sm:p-6">
      <div class="mb-6">
        <h2 class="text-2xl">Estimation mentuelle</h2>
        <p class="text-gray-600 dark:text-gray-400 text-sm">
          Vous payez ce que vous consommez : facturation à la seconde, sans
          frais initiaux.
        </p>
      </div>
      ...
      <div class="mt-5">
        <div
          class="rounded-md bg-gray-50 px-6 py-5 sm:flex sm:items-start sm:justify-between bg-blue-50 dark:bg-gray-800"
        >
          ...
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 sm:ml-4 sm:mt-0">
              ...
              <div
                class="mt-1 text-sm text-gray-600 sm:flex sm:items-center dark:text-gray-400 whitespace-nowrap"
              >
          ...
          <div class="mt-4 sm:ml-6 sm:mt-0 sm:shrink-0">
            <c-button size="sm">Modifier</c-button>
          </div>
        </div>
      </div>
    </div>
      ...
      <dl v-if="props.price?.cpu" class="mt-6 space-y-4">
        <div class="flex items-center justify-between">
          ...
          <dd class="text-sm font-medium text-gray-900 dark:text-gray-200">
            € {{ formatPrice(props.price?.cpu) }} /h
          </dd>
        </div>
        <div
          v-if="props.price?.ram"
        ...
        >
          ...
          <dd class="text-sm font-medium text-gray-900 dark:text-gray-200">
            € {{ formatPrice(props.price?.ram) }} /h
          </dd>
        </div>
        <div
          v-if="props.price?.totalHourlyPrice.amount"
          class="flex items-center justify-between border-t border-gray-200 dark:border-gray-800 pt-4"
        >
          ...
          <dd class="text-sm font-medium text-gray-900 dark:text-gray-200">
            €
            {{
              formatPrice(props.price?.totalHourlyPrice.amount * 24 * 30 * 0.2)
            }}
          </dd>
        </div>
        <div
          ...
          class="flex items-center justify-between border-t border-gray-200 dark:border-gray-800 pt-4"
        >
          <dt class="text-base font-medium text-gray-900 dark:text-gray-200">
            Total
          </dt>
          <dd class="text-base font-medium text-gray-900 dark:text-gray-200">
            € {{ formatPrice(props.price?.totalHourlyPrice.amount * 24 * 30) }}
          </dd>
        </div>
      </dl>

      <div class="mt-6">
        <c-button block @click.stop="$emit('click')" :loading="props.loading"
          >Commander</c-button
        >
      </div>
    </section>
...
interface Props {
  loading: boolean;
  price: {
    totalHourlyPrice: { amount: number };
    cpu?: number;
    ram?: number;
    disk?: number;
  } | null;
