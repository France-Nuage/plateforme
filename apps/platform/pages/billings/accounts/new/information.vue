<template>
  <c-card class="w-11/12 md:w-8/12 lg:w-4/12 mx-auto mt-24">
    ...
    <c-card-body>
      <p class="text-xs mb-8 dark:text-gray-400">
        <span class="font-semibold"
          >This is your payment account within France-Nuage.</span
        >
    ...
    <c-card-footer>
      <div>
        <c-button variant="filled" size="sm" @click="router.go(-1)"
          >Annulé</c-button
        >
      </div>
      <div class="flex items-center gap-4">
        <small class="text-xs dark:text-gray-400"
          ...
          du compte de paiement plus tard</small
        >
        <c-button size="sm" @click="onSubmit" :loading="loading"
          >Créer un compte de paiement</c-button
        >
      </div>
    </c-card-footer>
  </c-card>
