<template>
  <c-drawer v-model="isOpen" title="plop">
    <template #header>
      <c-drawer-header title="Modifier l'accès à l'organization" />
    </template>

    <div class="flex flex-col gap-4">
      <div
        class="relative flex items-center space-x-6 rounded-lg border px-6 py-5 shadow-sm border-gray-300 bg-white hover:border-gray-400 dark:bg-gray-800 dark:border-gray-700"
      >
        <div class="shrink-0">
          <c-avatar size="xl" />
        </div>
      ...
      <c-alert variant="information" title="A noter">
        <div class="mb-3">
          Les rôles sont composés d'ensemble d'autorisations et déterminent ce
          que le compte peut faire avec cette resource.
        </div>
          ...
          <div class="min-w-0 w-full">
            <div class="flex justify-between items-start gap-x-3 w-full">
              <div class="flex-1 flex items-center gap-x-3">
                ...
                />
                <p
                  class="text-sm/6 font-normal text-gray-900 dark:text-gray-400"
                >
