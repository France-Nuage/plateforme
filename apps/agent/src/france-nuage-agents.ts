...
async function withRetry<T>(
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            if (attempt === retries) {
                Logger.error(`${operationName} failed after ${retries} attempts. Last error: ${lastError.message}`);
                throw lastError;
            }

            Logger.warn(`${operationName} failed (attempt ${attempt}/${retries}). Retrying in ${delay/1000}s...`);
            await sleep(delay);
        }
    }
...
async function collectMetrics(): Promise<MetricData> {
    return withRetry(async () => {
        ...
        const diskUsage = child_process.execSync('df -h /').toString().split('\n')[1].split(/\s+/);
    ...
    }, MAX_RETRIES, RETRY_DELAY, 'Metric collection');
}
...
async function getServerInfo(): Promise<ServerInfo> {
    return withRetry(async () => {
        ...
        const installed_packages = child_process.execSync('npm list -g --depth=0').toString().split('\n');
        ...
        const quotas: QuotasInfo = {
            ...
            memoryUsagePercent: ((total_memory - os.freemem()) / total_memory) * 100,
            diskUsagePercent: parseFloat(child_process.execSync('df -h /').toString().split('\n')[1].split(/\s+/)[4])
        };
    ...
    }, MAX_RETRIES, RETRY_DELAY, 'Server info collection');
}
...
async function sendMetrics(data: MetricData): Promise<void> {
    ...
    }, MAX_RETRIES, RETRY_DELAY, 'Metrics sending');
}
...
async function checkApiAvailability(): Promise<void> {
    await withRetry(async () => {
        ...
        if (response.status !== 200) {
            throw new Error(`API is not reachable. Status code: ${response.status}`);
        }

        Logger.info('API is reachable.');
    }, MAX_RETRIES, RETRY_DELAY, 'API availability check');
}
/**
    ...
    try {
        ...
        setInterval(async () => {
            try {
                ...
                Logger.error(`Error in metrics collection cycle: ${error instanceof Error ? error.message : String(error)}`);
            }
        }, Number(INTERVAL));
