...
pub struct ManagedServiceVersion {
    ...
    pub connection_info_schema: Option<Value>,
    #[fabrique(soft_delete)]
    pub deactivated_at: Option<DateTime<Utc>>,
    pub created_at: DateTime<Utc>,
}
...
fn deep_merge(base: &mut Value, overlay: &Value) {
    ...
    let overlay_map = overlay.as_object().expect("checked above");
...
pub enum ManagedServiceError {
    ...
    #[error("plan {0} requires payment, use the billing checkout flow")]
    PlanRequiresPayment(String),
...
pub(crate) async fn transition_instance_status(
    ...
    // Raw SQL: multi-table join across lib_fsm schema not expressible with fabrique
    ...
    let Some(event_name) = event else {
        // Raw SQL: cross-schema join on lib_fsm not expressible with fabrique
    ...
    // Raw SQL: function call to lib_fsm.state_machine_transition not expressible with fabrique
...
pub fn build_instance_labels(
    ...
    BTreeMap::from([
        ...
        ("france-nuage/project".to_owned(), project_slug.to_owned()),
    ])
}
...
impl<A: Authorize> ManagedServices<A> {
    ...
    pub(crate) fn build_platform_values(&self, service: &ManagedService) -> Value {
        let spec = platform_values_spec(service);
        let storage_class = self.platform_config.default_storage_class.as_deref();
        let backup_enabled = self.platform_config.cnpg_backup_enabled;

        let mut map = serde_json::Map::new();
        if let Some(sc) = storage_class {
            set_nested(
                &mut map,
                spec.app_storage_class_path,
                Value::String(sc.to_owned()),
            );
            if let Some(db_path) = spec.db_storage_class_path {
                set_nested(&mut map, db_path, Value::String(sc.to_owned()));
            }
        }
        if let Some(backup_path) = spec.backup_enabled_path {
            set_nested(&mut map, backup_path, Value::Bool(backup_enabled));
        }
        Value::Object(map)
    }
    ...
    pub async fn register_version(
        ...
        connection_info_schema: Option<&Value>,
        ...
        // fabrique limitation: ON CONFLICT on non-PK unique constraint (service_id, chart_version)
        let version = sqlx::query_as::<_, ManagedServiceVersion>(
            r#"INSERT INTO managed.service_version
                   (id, service_id, chart_version, app_version, oci_reference,
                    configurable_values_schema, ui_schema, connection_info_schema)
    ...
    pub async fn get_instance_checked<P: Principal + Sync>(
        &self,
        principal: &P,
        instance_id: Uuid,
    ) -> Result<ManagedServiceInstanceView, ManagedServiceError> {
        ...
        self.find_instance_with_status(instance_id).await
    }

    pub async fn get_instance_for_connection_info<P: Principal + Sync>(
        &self,
        principal: &P,
        instance_id: Uuid,
    ) -> Result<ManagedServiceInstance, ManagedServiceError> {
    ...
    pub async fn list_instances_checked<P: Principal + Sync>(
        &self,
        principal: &P,
        project_slug: &str,
    ) -> Result<Vec<ManagedServiceInstanceView>, ManagedServiceError> {
        let slug = project_slug.to_owned();
        self.auth
            .can(principal)
        ...
        self.list_instances_by_project(project_slug).await
    }
...
fn set_nested(root: &mut serde_json::Map<String, Value>, path: &[&str], value: Value) {
    let (last, segments) = path.split_last().expect("path must not be empty");
    ...
    for segment in segments {
        current = current
            ...
            .as_object_mut()
            .expect("intermediate path must be an object");
    }
    current.insert((*last).to_owned(), value);
}

struct PlatformValuesSpec {
    app_storage_class_path: &'static [&'static str],
    db_storage_class_path: Option<&'static [&'static str]>,
    backup_enabled_path: Option<&'static [&'static str]>,
}

fn platform_values_spec(service: &ManagedService) -> PlatformValuesSpec {
    PlatformValuesSpec {
        app_storage_class_path: &["persistence", "storageClass"],
        db_storage_class_path: if service.database_engine.is_some() {
            Some(&["postgres", "storageClass"])
        } else {
            None
        },
        backup_enabled_path: if service.database_engine.is_some() {
            Some(&["postgres", "backup", "enabled"])
        } else {
            None
        },
    }
