...
impl KubernetesLabels {
    ...
    pub async fn attach_label<P: Principal + Sync>(
        ...
        Self::insert_attachment(&self.db, cluster_id, label_id).await
    }
    ...
    pub async fn attach_labels<P: Principal + Sync>(
        ...
        Self::batch_attach(&self.db, cluster_id, label_ids).await
    }

    /// Like [`Self::attach_labels`] but writes through the provided
    /// connection. Labels must already be validated via
    /// [`Self::ensure_user_labels_exist`].
    pub async fn attach_labels_on<P: Principal + Sync>(
        &self,
        principal: &P,
        cluster_id: Uuid,
        label_ids: &[Uuid],
        ...
        require_admin(principal)?;
        Self::batch_attach(&mut *conn, cluster_id, label_ids).await
    }

    /// Single-query batch insert, idempotent via ON CONFLICT DO NOTHING.
    async fn batch_attach<'e, E: sqlx::Executor<'e, Database = Postgres>>(
        executor: E,
        cluster_id: Uuid,
        label_ids: &[Uuid],
    ) -> Result<(), KubernetesLabelError> {
        ...
        // constraint, which the query builder cannot express. unnest turns the
        // array into rows so all labels attach in a single round-trip.
        sqlx::query(
    ...
    async fn insert_attachment<'e, E: sqlx::Executor<'e, Database = Postgres> + 'e>(
        executor: E,
        cluster_id: Uuid,
        label_id: Uuid,
    ) -> Result<(), KubernetesLabelError> {
