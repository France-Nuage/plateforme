...
impl hypervisor::Instance for VM<'_, '_> {
    ...
    async fn delete(&self) -> Result<(), hypervisor::error::Error> {
        crate::endpoints::vm_delete(self.api_url, self.client, self.node.id, self.id).await?;
        Ok(())
    }

    /// Starts the instance.
    async fn start(&self) -> Result<(), hypervisor::error::Error> {
        crate::endpoints::vm_status_start(self.api_url, self.client, self.node.id, self.id).await?;
        Ok(())
    }

    /// Gets the instance status.
    async fn status(&self) -> Result<hypervisor::InstanceStatus, hypervisor::error::Error> {
        let result =
    ...
    /// Stops the instance.
    async fn stop(&self) -> Result<(), hypervisor::error::Error> {
