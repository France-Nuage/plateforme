...
impl InstanceService for ProxmoxInstanceService {
    ...
    async fn create(&self, options: InstanceConfig) -> Result<String, Problem> {
        let next_id = crate::proxmox_api::cluster_next_id(&self.api_url, &self.client)
            .await
        ...
        let result =
            crate::proxmox_api::vm_create(&self.api_url, &self.client, "pve-node1", &vm_config)
                .await
    ...
    async fn delete(&self) -> Result<(), Problem> {
        crate::proxmox_api::vm_delete(&self.api_url, &self.client, "pve-node1", self.id).await?;
        Ok(())
    }
