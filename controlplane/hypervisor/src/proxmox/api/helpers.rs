...
pub async fn wait_for_task_completion(
    ...
    node: &str,
    task: &str,
) -> Result<TaskStatusResponse, Error> {
    ...
    Retry::spawn(strategy, || async {
        let result =
            crate::proxmox::api::task_status_read(api_url, client, authorization, node, task).await;

        match result {
            Ok(response) => match response.data.status {
                TaskStatus::Running => Err(Error::TaskNotCompleted(task.to_owned())),
...
pub async fn get_vm_execution_node(
    ...
    let resource =
        crate::proxmox::api::cluster_resources_list(api_url, client, authorization, "vm")
            .await?
            .data
            .into_iter()
            .filter(|resource| {
