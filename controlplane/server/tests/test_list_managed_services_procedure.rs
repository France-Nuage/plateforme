use crate::common::{Api, deactivate_managed_service, seed_managed_service};
...
async fn test_list_managed_services_returns_active_services(
    ...
    let slugs: Vec<&str> = resp.services.iter().map(|s| s.slug.as_str()).collect();
    assert!(slugs.contains(&"nextcloud"));
    assert!(slugs.contains(&"vaultwarden"));
...
async fn test_list_managed_services_excludes_deactivated(
    ...
    assert_eq!(resp.services[0].slug, "vaultwarden");
