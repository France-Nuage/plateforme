...
use frn_rpc::v1::workflow::{Initiator, NextRequest, ScheduleRequest, initiator};
use serde_json::json;
use tonic::Request;
...
async fn test_next_returns_empty_when_no_executions(
    ...
    let resp = response.expect("next must succeed").into_inner();
...
async fn test_next_returns_pending_execution(
    ...
    let execution = next_resp
        .expect("next must succeed")
        .into_inner()
        .execution
        .expect("next must return an execution");
