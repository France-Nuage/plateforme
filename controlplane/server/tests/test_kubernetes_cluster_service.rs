...
async fn create_persists_cluster_with_health_info(pool: sqlx::PgPool) {
...
#[sqlx::test(migrations = "../migrations")]
async fn create_encrypts_kubeconfig(pool: sqlx::PgPool) {
    let service = service(&pool, false);
...
#[sqlx::test(migrations = "../migrations")]
async fn get_returns_persisted_cluster(pool: sqlx::PgPool) {
    let service = service(&pool, false);
    let admin = user(true);
