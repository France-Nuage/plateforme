...
mod tests {
    ...
    use workflow::operations::helm_install::HelmInstallError;
    use workflow::operations::update_instance_status::UpdateInstanceStatusError;
    use workflow::workflows::write_relationships::WriteRelationshipsWorkflow;

    fn execution(max_retry: i32) -> WorkflowExecution {
    ...
    /// A retryable error that does not consume the hard-retry budget.
    fn transient() -> Arc<dyn OperationError + Send + Sync> {
        Arc::new(UpdateInstanceStatusError::FsmDatabase(
            sqlx::Error::RowNotFound,
        ))
    }

    /// A violated invariant: never retried, fails the execution outright.
    fn invariant() -> Arc<dyn OperationError + Send + Sync> {
        Arc::new(UpdateInstanceStatusError::InvalidTransition(
            "bad".to_owned(),
        ))
    }

    /// A plain error that consumes the hard-retry budget.
    fn retryable() -> Arc<dyn OperationError + Send + Sync> {
        Arc::new(HelmInstallError::Failed("helm exited 1".to_owned()))
    }

    #[test]
    fn failure_always_increments_soft_try_count() {
        let mut exec = execution(3);

        handle_failure(&mut exec, &ProcessError::Timeout);

        assert_eq!(exec.soft_try_count, 1);
    }

    #[test]
    fn transient_operation_failure_does_not_consume_hard_retries() {
        let mut exec = execution(3);

        handle_failure(&mut exec, &ProcessError::OperationFailed(vec![transient()]));

        assert_eq!(
            (exec.status, exec.hard_try_count),
            (WorkflowExecutionStatus::WillRetry, 0)
        );
    }

    #[test]
    fn invariant_operation_failure_fails_despite_remaining_budget() {
        let mut exec = execution(10);

        handle_failure(&mut exec, &ProcessError::OperationFailed(vec![invariant()]));

        assert_eq!(exec.status, WorkflowExecutionStatus::Failed);
    }

    #[test]
    fn retryable_operation_failure_below_limit_will_retry() {
        let mut exec = execution(3);

        handle_failure(&mut exec, &ProcessError::OperationFailed(vec![retryable()]));

        assert_eq!(
            (exec.status, exec.hard_try_count),
            (WorkflowExecutionStatus::WillRetry, 1)
        );
    }

    #[test]
    fn retryable_operation_failure_exhausting_budget_fails() {
    ...
    #[test]
    fn dependency_failure_fails_and_consumes_a_hard_retry() {
        let mut exec = execution(5);

        handle_failure(
            &mut exec,
            &ProcessError::DependencyFailed(WorkflowExecutionId::new()),
        );

        assert_eq!(
            (exec.status, exec.hard_try_count),
            (WorkflowExecutionStatus::Failed, 1)
        );
    }

    #[test]
    fn generic_failure_below_limit_will_retry() {
        let mut exec = execution(3);
    ...
    #[test]
    fn generic_failure_at_limit_fails() {
        let mut exec = execution(2);
        exec.hard_try_count = 1;

        handle_failure(&mut exec, &ProcessError::MaxOperationRoundsExceeded);

        assert_eq!(exec.status, WorkflowExecutionStatus::Failed);
    }

    #[test]
    fn failure_schedules_a_retry_in_the_future() {
        let mut exec = execution(3);
        let before = Utc::now();

        handle_failure(&mut exec, &ProcessError::Timeout);

        assert!(exec.next_retry_at > before);
    }

    #[test]
    fn apply_completed_outcome_marks_completed() {
        let mut exec = execution(3);

        apply_outcome(&mut exec, ProcessOutcome::Completed);

        assert_eq!(exec.status, WorkflowExecutionStatus::Completed);
    }

    #[test]
    fn apply_waiting_outcome_records_dependencies_and_retry_time() {
        let mut exec = execution(3);
        let deps = vec![WorkflowExecutionId::new(), WorkflowExecutionId::new()];
        ...
        assert_eq!(
            (exec.status, exec.dependencies.clone(), exec.next_retry_at),
            (WorkflowExecutionStatus::WillRetry, deps, when)
        );
    }

    #[test]
    fn apply_scheduled_sub_workflows_outcome_records_dependencies() {
        let mut exec = execution(3);
        let deps = vec![WorkflowExecutionId::new()];

        apply_outcome(
            &mut exec,
            ProcessOutcome::ScheduledSubWorkflows {
                dependencies: deps.clone(),
            },
        );

        assert_eq!(
            (exec.status, exec.dependencies.clone()),
            (WorkflowExecutionStatus::WillRetry, deps)
        );
    }

    #[test]
    fn apply_failed_outcome_sets_status() {
        let mut exec = execution(3);
