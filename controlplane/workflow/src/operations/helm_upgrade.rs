...
impl crate::operations::Operation for HelmUpgradeOp {
    ...
    async fn execute(
        ...
        // prior install never completed, as having no deployed revision). Both
        // are benign for an idempotent retry, so they must not consume a hard
        ...
        .map_err(HelmUpgradeError::Failed)?
        {
    ...
    async fn rollback(
        ...
        // Helm rollback has no benign-failure marker: any non-zero exit is a
        // genuine failure.
        classify_helm_result(output.status.success(), &output.stderr, &[])
            .map_err(HelmUpgradeError::Failed)?;
