...
/// Labels are stored to allow rollback to recreate the namespace.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DeleteNamespaceOp {
...
impl crate::operations::Operation for DeleteNamespaceOp {
    ...
    async fn execute(
        ...
        match classify_delete(
            ...
            DeleteOutcome::Deleted => info!(namespace = %self.namespace, "namespace deleted"),
            DeleteOutcome::AlreadyGone => {
                info!(namespace = %self.namespace, "namespace already gone")
            }
        }
    ...
    async fn rollback(
        ...
        match classify_create(namespaces.create(&PostParams::default(), &ns).await)? {
            CreateOutcome::Created => {
                info!(namespace = %self.namespace, "namespace recreated (rollback)")
            }
            CreateOutcome::AlreadyExists => {
                info!(namespace = %self.namespace, "namespace already exists (rollback)")
            }
        }
