...
mod tests {
    ...
    fn definition(done: bool) -> WorkflowDefinitions {
        serde_json::from_value(json!({
            "WriteRelationships": { "relationships": [], "done": done }
        }))
        .unwrap()
    }
    ...
    fn idempotency_key_is_deterministic() {
        let parent = WorkflowExecutionId::new();
        let def = definition(false);

        let first = WorkflowService::compute_idempotency_key(parent, &def).unwrap();
        let second = WorkflowService::compute_idempotency_key(parent, &def).unwrap();

        assert_eq!(first, second);
    }

    #[test]
    fn idempotency_key_depends_on_parent() {
        let def = definition(false);

        let a = WorkflowService::compute_idempotency_key(WorkflowExecutionId::new(), &def).unwrap();
        let b = WorkflowService::compute_idempotency_key(WorkflowExecutionId::new(), &def).unwrap();

        assert_ne!(a, b);
    }

    #[test]
    fn idempotency_key_depends_on_definition() {
        let parent = WorkflowExecutionId::new();

        let a = WorkflowService::compute_idempotency_key(parent, &definition(false)).unwrap();
        let b = WorkflowService::compute_idempotency_key(parent, &definition(true)).unwrap();

        assert_ne!(a, b);
    }

    #[test]
    fn idempotency_key_is_a_valid_uuid() {
        let key = WorkflowService::compute_idempotency_key(
