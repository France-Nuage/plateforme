...
use crate::{error::Error, hypervisor::InstanceStatus, proxmox::ProxmoxResponse};
...
pub async fn vm_status_read(
    ...
    let result: ProxmoxResponse<VMStatus> = client
        .get(format!(
...
mod tests {
    ...
    async fn test_vm_status_read() {
        ...
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), InstanceStatus::Running);
    }
}
