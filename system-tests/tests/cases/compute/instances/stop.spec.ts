...
test.describe('Compute / Instances', () => {
  test('I can stop an existing instance', async ({ actingAs, pages, page, project }) => {
    ...
    await row.waitFor();
    await row.getByRole('button', { name: 'stop instance' }).click();
    await row.getByText('stopped').waitFor({ timeout: 120 * 1000 });
    await row.getByRole('button', { name: 'stop instance' }).waitFor({ state: 'detached' });
  });
});
